...
| `maxModelConcurrent` | Cap on in-flight monitored jobs (all models) |
| `local` | Local GPU seat: model id, `maxStreams` (default 1), optional preflight probe |
...
Edit model ids to match your Pi providers. Mark local entries with `"local": true`.
...
| ---------- | ------- | ------- |
| **easy** | Narrow cheap steps (summaries, scaffolds, small files) | Prefer local vLLM when a stream slot is free; overflow to the rest of the easy list |
| **medium** | Bulk implementation with disjoint write lanes | First medium catalog entry (or exact `model=`) |
| **hard** | Architecture, critique, VERIFY — **not** the default implementer | First hard catalog entry |

**Never** dump a whole project on one `difficulty=hard` spawn. Decompose; promote only when needed. Local busy on easy does **not** escalate to medium/hard — it falls through the easy list.
...
Shared context is **markdown only** — panes do not chat to each other. Each job gets a fresh session JSONL; panes stay open after success so you can watch or intervene.
...
Async jobs are monitored in the background. When a job finishes, the parent session gets a **herd-result** follow-up (not a long blocking wait). Use `herd wait` / `herd collect` when you need a sync barrier.
...
| Action | Purpose |
| ------ | ------- |
| `models` | Show catalog + local stream use |
...
Package skills (`herd`, `herdr`) teach the launcher the bottom-up mindset and the herd vs herdr split. They install with the package via the Pi `skills` manifest.
